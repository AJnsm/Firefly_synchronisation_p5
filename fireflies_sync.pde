...
void setup() {
  ...
  for (int i = 0; i<=blinkNum; i++) {
    ...
    blinkers.add(new Blinker(period*random(0.97, 1.03), count, pos));
  }
}
...
class Blinker {
  ...
  int period;      // determines the period
  ...
  float sensitivity = 0.15; // this is the variable that most determines the synchronisation-behaviour
